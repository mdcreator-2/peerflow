        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
          <button
            ...
            className={`p-4 rounded-lg text-left transition-colors ${
              filter === 'all' ?  'bg-primary-100 border-2 border-primary-500' : 'bg-white shadow hover:shadow-md'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg text-left transition-colors ${
              filter === 'pending' ? 'bg-yellow-100 border-2 border-yellow-500' : 'bg-white shadow hover:shadow-md'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg text-left transition-colors ${
              filter === 'confirmed' ? 'bg-blue-100 border-2 border-blue-500' : 'bg-white shadow hover:shadow-md'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg text-left transition-colors ${
              filter === 'completed' ? 'bg-green-100 border-2 border-green-500' : 'bg-white shadow hover: shadow-md'
            }`}
          >
          ...
          <button
            ...
            className={`p-4 rounded-lg text-left transition-colors ${
              filter === 'cancelled' ? 'bg-red-100 border-2 border-red-500' :  'bg-white shadow hover:shadow-md'
            }`}
          >
                  ...
                  <div className="p-6">
                      ...
                      {order.products.map((product, idx) => (
                        <div key={idx} className="flex items-center gap-4">
                          ...
                          <div className="text-right">
                            <p className="font-semibold text-gray-900">{formatPrice(product.total)}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="mt-6 pt-4 border-t border-gray-100">
                      ...
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span>{order.order_summary?. delivery_fee > 0 ? formatPrice(order.order_summary.delivery_fee) : 'Free'}</span>
                      </div>
                      <div className="flex justify-between text-lg font-bold">
                        <span>Total</span>
                        <span className="text-primary-600">{formatPrice(order.order_summary?.total_amount || 0)}</span>
                      </div>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium capitalize">
                          {order.delivery?. method === 'pickup' ? '📍 Campus Pickup' : '🚚 Delivery'}
                        </p>
                        {order.delivery?.pickup_location && (
                          <p className="text-gray-500">{order.delivery. pickup_location}</p>
                        )}
                      </div>
                      <div>
                        ...
                        <p className="font-medium capitalize">
                          {order.payment?.method === 'cash' ? '💵 Cash on Delivery' : 
                           order.payment?.method === 'upi' ? '📱 UPI' : '💳 Card'}
                        </p>
                        <p className={`text-xs ${
